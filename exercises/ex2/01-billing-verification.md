...
4. For this exercise, there is no need to download the Balance Statement, as we have provided screenshots for reference
5. In the same card, click the link "Important information about your billing and usage data".
6. You will see that the global account is licensed for both the consumption-based and subscription-based commercial models. This means that billing is structured so that the prepaid quota from your subscription is utilized first, after which any excess usage is charged under the consumption-based model.
7. In the header, locate the Cloud Platform Enterprise Agreement card (card on the right), which displays information about Cloud Credits, including costs, status, and trends.
8. In the Costs and Usage header, set the Month filter to the previous month.
...
- ✅ Distinguish between "View by Service" and "View by Account" perspectives

#### Billing Verification & Validation
- ✅ Compare Balance Statement data with BTP Cockpit information
- ✅ Verify service costs across billing sections (Total Consumption vs. CPEA)
- ✅ Calculate and validate cloud credit balances (allocated, used, remaining)
- ✅ Understand how prepaid quota vs. charged usage affects billing

#### Cost Analysis & Trend Identification
- ✅ Analyze global account cost trends over 12 months
- ✅ Identify cost anomalies and peak spending periods
- ✅ Interpret estimated vs. finalized costs in monthly billing cycles
